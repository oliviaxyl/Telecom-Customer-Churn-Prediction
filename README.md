...
According to the plot, training score (red line) decreases and plateau, which indicates underfitting with high bias. Cross-validation score (green line) stagnating throughout, and unable to learn from the data. The poor fitting could be explained by principle component visualization graphs, where pc scores show that there is no apparent separation between two clusters. To improve model accuracy, we can increase model complexity. Specifically, increase number of trees in the forest for AdaBoost classifer. However, if we're looking for a big improvement in performance, further work includes feature engineering and acquiring more attributes if feasible.
