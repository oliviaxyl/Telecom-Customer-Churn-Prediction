...
According to the plot, training score (red line) decreases and plateau, which indicates underfitting with high bias. Cross-validation score (green line) stagnating throughout, and unable to learn from the data. It could be explained by principle component visualization graph, and these scores show that there is no apparent separation between the two clusters. To further improve model accuracy, we can perhaps increase model complexity. More specifically, increase number of trees in the forest for AdaBoost classifer. However, if we're looking for a big improvement, more feature engineering may needed or we could acquire more attributes to add up info to the data.
